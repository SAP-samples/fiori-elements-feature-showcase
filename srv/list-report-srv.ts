  ...
  init() {
    ...
    this.after(["READ"], RootEntities, async (response) => {
      ...
      if (response.chartEntities && response.chartEntities.length > 0) {
        ...
        if (
          !response.chartEntities[0].forecastValue ||
          !response.chartEntities[0].targetValue
        ) {
          ...
          const chartEntities = new Map(
            (
              await SELECT.from(ChartDataEntities)
                .columns("ID", "forecastValue", "targetValue")
                .where({ parent_ID: response.ID })
            ).map((key) => [
              key.ID,
              {
                forecastValue: key.forecastValue,
        ...
        response.chartEntities.forEach((e) => {
          ...
          e.areaChartToleranceLowerBoundValue =
            e.integerValue -
            Math.round((e.integerValue / e.targetValue) * 5 + 15);
          e.areaChartDeviationUpperBoundValue =
            e.forecastValue +
            Math.round((e.integerValue / e.forecastValue) * 10 + 30);
          e.areaChartDeviationLowerBoundValue =
            e.forecastValue -
            Math.round((e.integerValue / e.forecastValue) * 10 + 30);
        });
      }
    ...
    //      Actions
    //=============================================================================================================
    const { changeCriticality, changeProgress, resetEntities } =
      RootEntity.actions;
    ...
    this.on(changeProgress, async (req) => {
      ...
      const headerID = (
        req.params[0] as { ID: string; IsActiveEntity: boolean }
      ).ID;
    ...
    this.on(resetEntities, async (req) => {
      ...
      for (let i = 0; i < countRootEntities; i++) {
        ...
        rootEntities.push({
          ...
          uom_code:
            i >= unitOfMeasures.length
              ? unitOfMeasures[0].code
              : unitOfMeasures[i].code,
          isoCurrency_code:
          ...
          stringProperty:
            i === 0
              ? `Root entity ${i + 1} and delete not possible`
              : i === 1
                ? `Root entity ${i + 1} and update not possible`
                : `Root entity ${i + 1}`,
    ...
    async function createChildEntities1(amount: number, parent_ID: string) {
      ...
      for (let i = 0; i < amount; i++) {
        ...
        childEntities1.push({
          ...
          criticalityValue_code:
            i % 2 === 0 ? 1 : i % 3 === 0 ? 2 : i % 5 === 0 ? 3 : 0,
