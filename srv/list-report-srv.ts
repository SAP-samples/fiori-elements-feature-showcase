...
export class LROPODataService extends cds.ApplicationService { init() {
    ...
    this.after(["READ"],RootEntities, async (response) => {
        ...
        if(response.contact && response.contact.addressLabel === null) {
            //Requesting the 
            const contact =  await SELECT.one.from(Contacts,response.contact.ID).columns(contact => {
        ...
        if(response.chartEntities && response.chartEntities.length > 0) {
            ...
            if(!response.chartEntities[0].forecastValue || !response.chartEntities[0].targetValue) {
                ...
                const chartEntities =  new Map((await SELECT.from(ChartDataEntities).columns('ID', 'forecastValue', 'targetValue').where({parent_ID : response.ID})).map(key => [key.ID, { forecastValue: key.forecastValue, targetValue: key.targetValue }]));
    ...
    this.before('NEW',RootEntities.drafts, async (req) => {
        ...
        for(let i = 1; i <= 10; i++) {
            req.data.chartEntities.push({
                ...
                //@ts-expect-error DraftAdministrativeData_DraftUUID is given on drafts but not yet in CDS type
                DraftAdministrativeData_DraftUUID : req.data.DraftAdministrativeData_DraftUUID
            });
        }
    })

    const { copyOrgUnit, moveOrgUnit } = OrganizationalUnit.actions;

    this.on(copyOrgUnit, async req => {
        //@ts-expect-error highestID is based on the SQL
        const [toCopy, {highestID}] = await Promise.all([
            SELECT.one.from(req.subject),
            SELECT.one.from(OrganizationalUnits).columns('max(externalId) as highestID')
        ]);
    ...
    //      Actions
    //=============================================================================================================
    const { changeCriticality, changeProgress, resetEntities } = RootEntity.actions;
    ...
    this.on(changeCriticality, async (req) => {
        ...
        return UPDATE(req.subject).with({criticality_code : criticality_code, fieldWithCriticality : determineFieldWithCriticalityValue(criticality_code)});
    });

    this.on(changeProgress, async (req) => {
        const integerValue = req.data.newProgress;
        // The alternative to using req.subject, is using the parameters of the request, which includes just! the keys of the entity
        const headerID = (req.params[0] as {ID: string, IsActiveEntity: boolean}).ID;
    ...
    this.on(unboundAction, async req => {
    ...
    this.on(criticalAction, async req => {
    ...
    this.on(resetEntities, async req => { 
        ...
        for(let i = 0; i < countRootEntities; i++) {
            ...
            rootEntities.push({
                ...
                fieldWithCriticality: determineFieldWithCriticalityValue((i === criticaityCodes.length) ? criticaityCodes[0].code : criticaityCodes[i].code),
                integerValue: 20+2*i,
    ...
    function determineFieldWithCriticalityValue(criticality_code: number): string {
    ...
    async function createChildEntities1(amount: number, parent_ID: string) {
        const childEntities1 = [];
        for(let i = 0; i < amount; i++) {
            const grandChildEntities = [];
    ...
    async function createChildEntities3(amount: number, parent_ID: string) {
        const childEntities3 = [];
        for(let i = 0; i < amount; i++) {
            childEntities3.push({
                ID: cds.utils.uuid(),
                parent_ID: parent_ID,
    ...
    async function createChartEntities(amount: number, parent_ID: string) {
        const chartEntities = [];
            for(let i = 1; i <= amount; i++) {
                chartEntities.push({
                    ID : cds.utils.uuid(),
